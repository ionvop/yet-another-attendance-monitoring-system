...
interface Props {
  initialData?: { name: string; start_time: string; end_time: string };
  onSubmit: (data: { name: string; start_time: string; end_time: string }) => Promise<void>;
  onCancel: () => void;
  saving: boolean;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            required
          />
          {errors.end_time && <p className="text-sm text-destructive">{errors.end_time[0]}</p>}
        </div>
      </div>
      <div className="flex justify-end gap-3 pt-2">
        ...
        <Button type="submit" disabled={saving || !name.trim() || !startTime || !endTime}>
          {saving ? "Saving..." : "Save"}
        </Button>
      </div>
    </form>
